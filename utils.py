...
def load_unimodal_weights(model, state_ckpt_path, action_ckpt_path, device):
    ...
    Either path can be None — that modality keeps its random initialization.
    ...
    if state_ckpt_path is not None:
        ...
        print(f"[Surgery] State Encoder: skipped (random init)")

    state_dict_a = None
    if action_ckpt_path is not None:
        ...
        print(f"[Surgery] Action Encoder: skipped (random init)")
    ...
    for key in current_model_dict.keys():
        ...
        if key.startswith('state_encoder') or key.startswith('state_embed') or key.startswith('state_encoder_norm'):
            if state_dict_s is not None and key in state_dict_s:
        ...
        # CASE 2: Action Encoder
            ...
            if state_dict_a is not None and key in state_dict_a:
