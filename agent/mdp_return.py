...
class MaskingEvalAgentMultimodal:
    """
    Eval agent for the hierarchical MaskedDPMultimodal model (proprioceptive only —
    this branch has no pixel_encoder / use_pixel_obs, unlike hierarchical-vd4rl).

    Protocol identical to MaskingEvalAgent (unimodal, MaskDP_paper) and to
    MaskingEvalAgentMultimodal in hierarchical-vd4rl:
        ...
        Eval    (rest of episode): context fixed at T_cond, predict T_pred actions per
                cycle, execute replan_freq of them before replanning. Return is measured.
    """
    ...
    def _forward(self):
        ...
        # Pos embeddings (interleaved positions of each real token)
        ...
        # Embed state tokens — propioceptivo puro, sin pixel_encoder
        obs_np = np.stack(list(self._obs_buffer))                   # (n_s, obs_dim)
        obs_t  = torch.as_tensor(
            obs_np, dtype=torch.float32, device=self.device
        ).unsqueeze(0)                                               # (1, n_s, obs_dim)
        s_real = self.mdp.state_embed(obs_t)                        # (1, n_s, enc_D)
        ...
        # Action encoder (unimodal stream)
        if n_a > 0:
        ...
        # Fusion (cross/self-attention between state and action streams)
