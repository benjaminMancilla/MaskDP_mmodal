...
from agent.mdp_return import MaskingEvalAgentMultimodal
...
def eval_masking(
    ...
    for episode in range(num_eval_episodes):
        ...
        while not time_step.last():
            obs = time_step.observation  # (obs_dim,) — propioceptivo puro
            ...
            episode_return_full += time_step.reward
            episode_length      += 1

            # Post-warmup return
            if episode_length > T_cond:
                episode_return_postwarmup += time_step.reward
...
def main(cfg):
    ...
    # Environment — propioceptivo puro, sin obs_type/pixel_size/frame_stack
    # (esta rama no tiene soporte de pixeles, igual que eval_goal.py aqui)
    env = dmc.make(cfg.task, seed=cfg.seed, action_repeat=cfg.get("action_repeat", 1))
