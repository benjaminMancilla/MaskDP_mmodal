...
def main(cfg):
    ...
    assert env_name in PROCGEN, f"{env_name} is not in the PROCGEN normalization dict."
    assert distribution_mode in PROCGEN[env_name], (
        f"distribution_mode='{distribution_mode}' not defined for env_name='{env_name}' in PROCGEN."
    )
